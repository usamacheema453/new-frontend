// components/brain/UploadManuals.js
...
export default function UploadManuals({ 
    ...
    const loadUserPlan = async () => {
      if (!propUserPlan) {
        try {
          ...
          console.log('📚 UploadManuals: Loaded user plan from storage:', finalPlan);
        } catch (error) {
          console.error('📚 UploadManuals: Error loading user plan:', error);
          setUserPlan('free');
        } finally {
        ...
        setPlanLoading(false);
        console.log('📚 UploadManuals: Using prop user plan:', propUserPlan);
      }
    };
  ...
  // Check if user has access to manual uploads
  useEffect(() => {
    if (!planLoading) {
      const hasUploadAccess = hasFeatureAccess(userPlan, FEATURES.UPLOAD_MANUALS);
      if (!hasUploadAccess) {
        setRestrictedFeature(FEATURES.UPLOAD_MANUALS);
  ...
  const addDemoFiles = () => {
    const demoFiles = [
      {
        id: Date.now() + 1,
        name: 'User_Manual_v2.1.pdf',
        size: 2456789,
        type: 'application/pdf',
        uri: 'demo://user-manual.pdf',
        uploadedAt: new Date().toISOString(),
      },
      {
        id: Date.now() + 2,
        name: 'Installation_Guide.docx',
        size: 876543,
        type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
        uri: 'demo://installation-guide.docx',
        uploadedAt: new Date().toISOString(),
      },
      {
        id: Date.now() + 3,
        name: 'Troubleshooting_Tips.txt',
        size: 123456,
        type: 'text/plain',
        uri: 'demo://troubleshooting.txt',
        uploadedAt: new Date().toISOString(),
      }
    ];
  ...
  const uploadFiles = async () => {
    if (selectedFiles.length === 0) {
      Alert.alert('No Files', 'Please select manual files first.');
      return;
    }

    if (!title.trim()) {
      Alert.alert('Title Required', 'Please enter a title for your manuals.');
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        'Upload Complete! 🎉',
        `Successfully uploaded ${selectedFiles.length} manual(s) to your brain! ${userPlan === 'free' ? `\n\nFree plan: ${currentUploads + 1}/${uploadAccess.uploadLimit} uploads used this month.` : ''}`,
        [
          { 
            text: 'Add More Manuals', 
            onPress: resetForm
          },
          { 
            text: 'Done', 
      ...
      Alert.alert('Upload Failed', 'Failed to upload manuals. Please try again.');
    } finally {
  ...
  const getFileIcon = (mimeType) => {
    if (mimeType === 'application/pdf') {
      ...
      return { icon: FileText, color: '#059669' };
    }
    return { icon: FileText, color: '#6B7280' };
  };

  const renderFileItem = ({ item }) => {
    const { icon: IconComponent, color } = getFileIcon(item.type);
      ...
      <View style={styles.fileItem}>
        ...
        <View style={styles.fileDetails}>
          ...
          <Text style={styles.fileType}>
            {item.type?.split('/').pop()?.toUpperCase() || 'Unknown'}
          </Text>
        </View>
        ...
        <View style={[styles.header, { paddingTop: insets.top + 16 }]}>
          ...
          <View style={styles.headerCenter}>
            ...
            <View style={styles.headerTitleContainer}>
              <Text style={styles.headerTitle}>Upload Manuals</Text>
              {/* Plan indicator */}
        ...
        >
          ...
          <View style={styles.formSection}>
            <View style={styles.inputGroup}>
              ...
              <TextInput
                ...
                placeholder="Give your manuals a descriptive title..."
                value={title}
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput
                ...
                placeholder="Describe what these manuals cover (optional)..."
                value={description}
              ...
              <View style={styles.selectedHeader}>
                <View>
                  ...
                  <Text style={styles.totalSize}>
                    Total size: {formatFileSize(selectedFiles.reduce((total, file) => total + (file.size || 0), 0))}
                  </Text>
                </View>
          ...
          {selectedFiles.length === 0 && (
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyTitle}>No manuals selected</Text>
              <Text style={styles.emptyDescription}>
                Select PDF, Word, or text files containing manuals, guides, and documentation
              </Text>
              {userPlan === 'free' && (
          ...
          <View style={styles.infoSection}>
            ...
            <View style={styles.tipBox}>
              ...
              <Text style={styles.tipText}>
                ...
                • Update outdated manuals regularly
              </Text>
            </View>
